...
export class S3Service {
   ...
   */
  private generateFileKey(originalName: string, customerId: string, category?: string): string {
  ...
  public async uploadFile(
    ...
    try {
      const key = this.generateFileKey(originalName, customerId, options.metadata?.category);
      const contentType = options.contentType || mime.lookup(originalName) || 'application/octet-stream';
  ...
  public async generateDownloadUrl(
    ...
    try {
      ...
      s3Logger.error('Generate download URL failed', {
        key,
        error: error instanceof Error ? error.message : 'Unknown error',
      });
