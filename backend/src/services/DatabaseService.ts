...
export class DatabaseService {
  ...
  private setupEventListeners(): void {
    ...
    // Database logging will be handled through other means
  }
  ...
  public async batchOperation<T>(
    operations: Array<(prisma: Omit<typeof this.prisma, '$connect' | '$disconnect' | '$on' | '$transaction' | '$use' | '$extends'>) => Promise<T>>
  ): Promise<T[]> {
    try {
      const results = await this.prisma.$transaction(async (prisma) => {
        const promises = operations.map(op => op(prisma));
