...
import { errorLogger, logSecurityEvent } from '@/utils/logger';
...
function handleMulterError(error: MulterError): ApiError {
  switch (error.code) {
    ...
    case 'MISSING_FIELD_NAME':
      return ApiError.badRequest('Missing field name');
...
function logError(
  ...
  if (apiError.statusCode >= 500) {
    ...
    // Other errors - info
    errorLogger.info('Error occurred', errorContext);
  }

  // Log specific error types with more detail
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    errorLogger.error('Prisma database error', {
      ...errorContext,
  ...
  if (error.name === 'JsonWebTokenError' || error.name === 'TokenExpiredError') {
    errorLogger.warn('JWT authentication error', {
      ...errorContext,
      jwtError: error.name,
    });
  }
