...
export const performanceMonitor = (
  ...
  res.on('finish', () => {
    ...
    if (duration > config.SLOW_REQUEST_THRESHOLD) {
      performanceLogger.warn('Performance: Slow request', performanceData);
    } else {
      performanceLogger.debug('Performance: Request completed', performanceData);
    }
  });
...
export const securityLogger = (
  ...
  if (isSuspicious) {
    requestLogger.warn('Suspicious request detected', {
      requestId: req.requestId,
      method: req.method,
      url: fullUrl,
      ip: getClientIP(req),
...
export const apiUsageTracker = (
  ...
  res.on('finish', () => {
    ...
    requestLogger.info('API usage tracked', usageData);
  });
...
export const requestSizeLogger = (
  ...
  if (contentLength > config.MAX_REQUEST_SIZE) {
    requestLogger.warn('Large request detected', {
      requestId: req.requestId,
      method: req.method,
      path: req.path,
      contentLength,
...
export const requestLoggerErrorHandler = (
  ...
  requestLogger.error('Request logging error', {
    requestId: req.requestId,
    error: error.message,
    stack: error.stack,
    method: req.method,
