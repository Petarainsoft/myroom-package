...
const envSchema = z.object({
  ...
  ALLOWED_FILE_TYPES: z.string().default('.glb,.gltf,.fbx,.obj,.3ds,.dae,.blend,.ma,.mb,.max,.c4d'),
  ...
  CACHE_TTL_CUSTOMER: z.string().transform(Number).default('60'),
...
export const derivedConfig = {
  ...
  cacheTTL: {
    ...
    customer: config.CACHE_TTL_CUSTOMER,
